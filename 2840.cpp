...
string printWheel(int n, int index, vector<char>& wheel) {
	...
	for (int i = index + n; i > index; i--) {
		ans += wheel[i % n]; // 현재 인덱스에서 시계방향으로 알파벳을 추가!
	}
	return ans;
}
...
string makeWheel(int n, int k, vector<ic>& record) {
	...
	int index = 0; //화살표가 가리키는 인덱스 초기화하기
